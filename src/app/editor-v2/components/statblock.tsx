...
import Statistic from "@/components/statblock/statistic";
import Actions from "@/components/statblock/actions";
...
const Statblock = ({ form, loadCreatureValues }: StatblockProps) => {
	const { selectedCreature, setCreature, updateCreature, creature } =
		useCreaturesStore();
	...
	useEffect(() => {
		updateCreature({ ...values });
	}, [updateCreature, values.name]);
	...
	const exp = values.challenge_rating
		? CHALLENGE_RATINGS.find(
				(rating) => rating.value === values.challenge_rating
		  )
		: CHALLENGE_RATINGS.find(
		...
		<div className="w-full print:columns-2 text-cararra-950 space-y-3">
			...
			<div className="space-y-1">
				...
				<div className="flex gap-2">
					<h4>Armor Class</h4>
					<p>
						{values.armor_class || creature.armor_class}{" "}
						{values.armor_desc
							? `(${values.armor_desc})`
			...
			<div className="grid grid-cols-6 w-fit gap-6 ">
				<Statistic stat="STR" value={values.strength || creature.strength} />
				<Statistic stat="DEX" value={values.dexterity || creature.dexterity} />
				<Statistic
					stat="CON"
					value={values.constitution || creature.constitution}
				/>
				<Statistic
					stat="INT"
					value={values.intelligence || creature.intelligence}
				/>
				<Statistic stat="WIS" value={values.wisdom || creature.wisdom} />
				<Statistic stat="CHA" value={values.charisma || creature.charisma} />
			</div>
			<Divider />
			<div>
				<SavingThrows
				...
				<p className="font-yatra">
					Senses{" "}
					<span className="font-sans">{values.senses || creature.senses}</span>
				</p>
				<p className="capitalize font-yatra">
